...
IUSE="debug doc examples +readline php python ruby test"
RESTRICT="test"
...
src_prepare() {
	...
	sed -i \
		-e 's|install-common||' \
		{cpp,php,python,ruby}/Makefile || die
...
src_configure() {
	...
	sed -i \
		...
		config/Make.rules.Linux python/config/Make.rules || die
...
src_install() {
	...
	use doc && DOCS+=( "${DISTDIR}/${PN}-3.7.6.pdf" )
	einstalldocs
